...
Public Class CDB
    Public Function OpenDB() As Boolean
        ...
        If objSQLConn Is Nothing Then 'we need to instantiate the connection object
            ...
            End Try
        Else    'under else, the conn will be open. But the conn can have different states
    ...
    Public Sub CloseDB()
        Try
            objSQLConn.Close() 'close the connection to the database. The connection is the pipeline through which we send commands to the database
        Catch ex As Exception
            MessageBox.Show("Error attempting to close database: " & ex.ToString, "Database error", MessageBoxButtons.OK, MessageBoxIcon.Error) 'always dump out the entire error message when dealing with DB
        End Try
    End Sub
    ...
    Public Function GetDataReaderBySP(ByVal strSP As String, ByRef params As ArrayList) As SqlDataReader
        If Not OpenDB() Then
            ...
            Return Nothing
        End If
        objSQLCommand = New SqlCommand(strSP, objSQLConn) 'takes strored procedure and conn pipeline
        objSQLCommand.CommandType = CommandType.StoredProcedure
        If Not params Is Nothing Then 'add each parameter into the command object
            For Each p As SqlParameter In params
                objSQLCommand.Parameters.Add(p) 'objSQLCommand has to be preloaded with sqlparams for this line to work
            Next
        End If
        Try
            Return objSQLCommand.ExecuteReader()    'get data from database and return it as a sqldatareader object
        Catch ex As Exception
    ...
    'functions also deal with update, insert, delete. These functions return whether the command was successful
    Public Function ExecSP(ByVal strSP As String, ByRef params As ArrayList) As Integer
        ...
        objSQLCommand = New SqlCommand(strSP, objSQLConn)
        objSQLCommand.CommandType = CommandType.StoredProcedure
        'this time it is not a store procedure. it is directly a command
        Try
            If Not params Is Nothing Then
                For Each p As SqlParameter In params
                    objSQLCommand.Parameters.Add(p)
                Next
            End If
            Return objSQLCommand.ExecuteNonQuery() 'bc it's not a query, test for > 0 (gt 0)
        Catch ex As Exception
